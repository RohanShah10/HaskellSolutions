...
reverseLookUp item lst = map fst (filter (\(c,d) -> d == item) lst)
...
inferType (Cond e1 e2 e3) table
    |(inferType e1 table) /= TBool = TErr
    |(inferType e2 table) /= (inferType e3 table) = TErr
    |otherwise = (inferType e2 table)

inferType (App e1 e2) table = inferApp (inferType e1 table) (inferType e2 table) table 
    where 
        inferApp :: Type -> Type -> TEnv -> Type
        inferApp (TFun t1 t') (t2) table
            |t1 == t2 = t' 
        inferApp t1 t2 table = TErr
...
applySubMap :: Sub -> [Type] -> [Type]
applySubMap sub = map (\x -> applySub sub x)  
...
unifyPairs (((TVar v), (t)):ts) s 
    |t /= (TVar v) && (not (occurs v t)) = unifyPairs i ((v,t) : s)
        where 
            i = map (\(x,y) -> (applySub [(v,t)] x, applySub [(v,t)] y)) ts
