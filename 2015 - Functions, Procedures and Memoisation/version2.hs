...
getValue id state = head (map (\(i,(s,v)) -> v) correctState)
    where 
        correctState = filter (\(i,(s,v)) -> i == id) (state)

getLocals :: State -> State
getLocals = filter (\(id, (scope, value)) -> scope == Local) 

getGlobals :: State -> State
getGlobals = filter (\(id, (scope, value)) -> scope == Global) 
...
assignArray (A arr) (I i) (I v) = let (f,s) = arr !! (i-1) in A ((take (i) (arr)) ++ (drop (i+1) arr) ++ [(f,v)])

updateVar :: (Id, Value) -> State -> State
updateVar (x, v) [] = [(x, (Local, v))]
...
applyOp (Add) (I n1) (I n2) = I (n1 + n2) 
applyOp (Mul) (I n1) (I n2) = I (n1 * n2)
applyOp (Less) (I n1) (I n2)
  |n1 < n2 = I n1
  |otherwise = I n2
applyOp (Equal) (I n1) (I n2)
  |n1 == n2 = I 1
  |otherwise = I 0
applyOp (Index) (A lst) (I n)
  |elem n (map fst lst) = I (lookUp n lst)
  |otherwise = I 0

bindArgs :: [Id] -> [Value] -> State
-- Pre: the lists have the same length
bindArgs is vs = map (\(a,b) -> (a, (Local, b))) (zip (is) (vs))

evalArgs :: [Exp] -> [FunDef] -> State -> [Value]
evalArgs es fs s = map (\e -> eval e fs s ) (es)
...
eval (Const c) fs s = c
eval (Var id) fs s = getValue id s
eval (Cond e1 e2 e3) fs s
  |eval e1 fs s == I 1 = eval e2 fs s
  |otherwise = eval e3 fs s
eval (OpApp o e1 e2) fs s = applyOp o (eval e1 fs s) (eval e2 fs s)
eval (FunApp id lst) fs s = undefined
    where 
      ase@ (as, e) = filter (\(i1, (lst, e)) -> i1 == id) fs
      vs = evalArgs lst fs s
      binded = bindArgs (map fst ase) vs
