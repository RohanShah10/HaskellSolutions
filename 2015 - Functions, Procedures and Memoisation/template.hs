...
getValue id state = lookUp id (convert state)
  where
    convert :: State -> [(Id,Value)]
    convert [] = []
    convert ((i,(s,v)) : xs) = (i,v) : convert xs

getLocals :: State -> State
getLocals [] = []
getLocals ((i,(s,v)) : xs) 
    |s == Local = (i,(s,v)) : getLocals xs
    |otherwise = getLocals xs
    
getGlobals :: State -> State
getGlobals [] = []
getGlobals ((i,(s,v)) : xs) 
    |s == Global = (i,(s,v)) : getGlobals xs
...
evalArgs [] _ _ = []
evalArgs (e:es) f s = (eval e f s) : evalArgs es f s
