...
simplify (Plus r) = (Seq (r) (Rep r))
simplify (Opt r) = (Alt r Null)
...
terminalStates (a,b,c) = b

transitions :: Automaton -> [Transition]
transitions (a,b,c) = c

isTerminal :: State -> Automaton -> Bool
isTerminal s a = elem s (terminalStates a)

transitionsFrom :: State -> Automaton -> [Transition]
transitionsFrom s a = filter (\(a,b,c) -> a == s) (transitions a)

labels :: [Transition] -> [Label]
labels ts =  nub (filter (/= Eps) thirds)
    where 
        thirds = map (\(a,b,c) -> c) ts
...
accepts a@(startingState, state_list, transition_list) str = accepts' startingState a str 

accepts' :: State -> Automaton -> String -> Bool 
accepts' state a null
    |isTerminal state a = True 

accepts' state a str = True `elem` map (\x -> try (str) (x) (a)) options
    where 
        options = transitionsFrom state a

try :: String -> Transition -> Automaton -> Bool 
try str (t,lst,Eps) a = accepts' t a str

try str (t,b, (C d)) a
  |null str = False
  |head str == d = accepts' t a (tail str)
  |otherwise = False
--------------------------------------------------------
-- Part III
...
make (Null) startState endState nextIdentifier =  undefined
