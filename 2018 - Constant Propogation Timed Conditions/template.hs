...
update (id, int) [] = [(id, int)]
update (id, int) state@(x:xs)
  |elem id (map fst state) = update' (id, int) state
  |otherwise = (id, int) : state
  where
    update' :: (Id, Int) -> State -> State
    update' (id, int) state@((f,s):xs)
      |f == id = (id, int) : xs
      |otherwise = (f,s) : update' (id, int) xs
...
execBlock [] _ = []
execBlock (b:bs) state = execStatement b state ++ execBlock bs state
