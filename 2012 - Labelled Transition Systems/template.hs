...
states :: LTS -> [State]
states [] = []
states lst@(((s1, s2), id):ts) = states' (nub (map fst lst ))
    where 
      states' :: [(Int, Int)] -> [State]
      states' lst = nub (concat (map (\(a,b) -> [a,b]) lst))

transitions :: State -> LTS -> [Transition]
transitions state = filter (\((a,b),c) -> a == state) 

alphabet :: LTS -> Alphabet
alphabet lts = nub (map snd lts)
...
actions (Choice lst) = concatMap actions lst
